"""
Kindle 閱讀筆記自動化工作流（固定範圍・上下文連貫版）
--------------------------------
與 fixed_region_watcher.py 的差別:翻譯時會帶上一頁的結尾當「上文脈絡」,
讓模型知道場景、說話的人是誰、上一句停在哪,譯文前後連貫:
  - 代名詞指涉不再用猜的(知道上一頁的「他」是誰)
  - 跨頁被切斷的句子能接得順
  - 語氣、稱謂、譯名延續前頁,不會每頁重新來過
代價是每次請求多帶一段上下文,token 用量增加、回應稍慢。
被擋後的分段補救「不帶」上下文(避免敏感上下文讓原本能過的內容也被擋)。
偵測到新章節時自動清空上下文(視角人稱常在換章時切換,帶舊脈絡反而誤導)。
...
執行：
  python fixed_region_watcher_context.py
  （不需要系統管理員身分，一般權限即可）
...
# 翻譯模型鏈:第一個是主力,過載/額度用完/無權限時依序往後退
MODEL_CHAIN = [
    "gemini-3.1-pro-preview",  # Google AI Pro 方案可用,譯文品質較好但回應較慢
...
# ==== 上下文連貫設定(本版新增) ====
USE_CONTEXT = True         # 翻譯時帶上一頁脈絡;改成 False 行為就跟原版一樣
CONTEXT_ORIG_TAIL = 400    # 帶入上一頁「原文」結尾的字元數(讓模型看到被切斷的句子)
CONTEXT_TRANS_TAIL = 500   # 帶入上一頁「譯文」結尾的字元數(讓模型延續語氣與譯名)
...
# ==== 滾動上下文(本版新增) ====
# 記住最近一次成功翻譯的頁面,翻下一頁時帶給模型當脈絡
_ctx_original = ""
_ctx_translated = ""


def _update_context(original: str, translated: str):
    global _ctx_original, _ctx_translated
    _ctx_original = original
    _ctx_translated = translated


def _clear_context(reason: str = ""):
    global _ctx_original, _ctx_translated
    if _ctx_original or _ctx_translated:
        _ctx_original = ""
        _ctx_translated = ""
        if reason:
            print(f"   🧹 已清空上下文({reason})")


def _build_prompt(text: str) -> str:
    """有上下文就組成「脈絡+新頁面」的完整輸入,沒有就直接回傳原文"""
    if not (USE_CONTEXT and _ctx_translated):
        return text
    parts = ["【上文脈絡】以下是前一頁的結尾,僅供銜接理解,不要重翻、不要輸出:"]
    if _ctx_original:
        parts.append("前頁原文結尾:" + _ctx_original[-CONTEXT_ORIG_TAIL:])
    parts.append("前頁譯文結尾:" + _ctx_translated[-CONTEXT_TRANS_TAIL:])
    parts.append("【新頁面】以下才是需要翻譯的內容,只輸出這部分的譯文:")
    parts.append(text)
    return "\n\n".join(parts)
...
def _is_model_unavailable(e: Exception) -> bool:
    """模型不存在或 API key 沒有權限用這個模型(例如 preview 模型未開放)"""
    msg = str(e)
    return any(k in msg for k in ("404", "NOT_FOUND", "403", "PERMISSION_DENIED"))
...
def _system_instruction() -> str:
    ...
    return (
        ...
        "如果輸入含有【上文脈絡】區塊，那是前一頁的結尾，僅供理解銜接："
        "請延續其人稱、稱謂、譯名與語氣，跨頁被切斷的句子要接得自然，"
        "但只翻譯並輸出【新頁面】區塊的內容，不要把脈絡部分重翻或輸出。"
...
def translate(text: str, chain: "list[str] | None" = None,
              use_context: bool = True) -> tuple[str, str]:
    """呼叫 Gemini API 翻譯:主模型失敗自動重試,重試用盡沿著模型鏈逐級換備援。
    chain 可指定這次要用的模型鏈,不給就用 MODEL_CHAIN。
    use_context=False 時不帶上一頁脈絡(分段補救路徑用,避免脈絡連累新內容被擋)。
    回傳 (譯文, 實際使用的模型名稱)"""
    models = chain or MODEL_CHAIN
    contents = _build_prompt(text) if use_context else text
    last_error = None
            ...
            try:
                resp = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        ...
                        # (pro 系列不允許 thinking_budget=0,會回 400,所以 pro 不設定)
                        thinking_config=(None if "pro" in model
                ...
                if not _is_transient_error(e):
                    # 模型拿不到(404/403,例如 preview 模型權限不足)就直接換下一個,重試沒有意義
                    if next_model and _is_model_unavailable(e):
                        print(f"   ⚠️ 無法使用 {model}（{e}），改用備援模型 {next_model}...")
                        break
                    raise
                if attempt < RETRY_COUNT:
                    ...
                    time.sleep(wait)
        if next_model and _is_transient_error(last_error):
            print(f"   🔁 {model} 持續過載，改用備援模型 {next_model}...")
    raise last_error


# 被擋後的分段補救要呼叫很多次 API:跳過又慢又容易再被擋的 pro,直接用 flash 系列
_RESCUE_CHAIN = [m for m in MODEL_CHAIN if "pro" not in m] or MODEL_CHAIN
...
def _translate_split(sentences: list[str], joiner: str, depth: int = 0) -> str:
    ...
    for _ in range(2):  # 硬性過濾的判定時好時壞,同樣內容多試常常就過
        try:
            # 補救路徑不帶上下文:上一頁的敏感內容可能連累原本能過的段落
            t, _ = translate(chunk, _RESCUE_CHAIN, use_context=False)
            return t
...
def process_page(img, last_text: str):
    ...
    try:
        ...
        _update_context(text, translated)
...
def _resume_context_from_notes():
    """啟動時把譯文檔的結尾當作初始上下文,重啟程式也能接續前頁的語氣與譯名"""
    if not (USE_CONTEXT and SEPARATE_FILES):
        return
    if not os.path.exists(TRANSLATION_FILE):
        return
    with open(TRANSLATION_FILE, "r", encoding="utf-8") as f:
        tail = f.read().rstrip()[-CONTEXT_TRANS_TAIL:]
    if tail:
        _update_context("", tail)
        print("🔗 已從譯文檔載入上一頁結尾當作上下文")


def main():
    print("📖 自動偵測翻頁模式已啟動（上下文連貫版）")
    _resume_chapter_from_notes()
    _resume_context_from_notes()
    print(f"監控範圍：{REGION}")
    if SEPARATE_FILES:
