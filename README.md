...
1. 到 [Google AI Studio](https://aistudio.google.com/) 申請 Gemini API Key，設定環境變數後重開終端機
2. 打開 `fixed_region_watcher.py` 設定區，改好筆記檔路徑、書名、章節
3. 執行 `python find_coordinates.py` 校準座標，把結果貼回 `REGION`
4. 執行 `python fixed_region_watcher.py`，開始閱讀

詳細步驟與疑難排解請見 [使用說明.md](使用說明.md)。

## 環境需求

- Windows 10/11（OCR 使用 winocr、通知使用 win11toast）
- Python 3.9+
- Gemini API Key（預設模型 `gemini-3.5-flash`，備援可改 `gemini-3.1-flash-lite`）
